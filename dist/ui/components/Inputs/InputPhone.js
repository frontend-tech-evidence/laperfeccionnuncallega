class InputPhone extends HTMLElement {
    constructor() {
        super();
                    ...
                    <div id="inputContainer${idCapitalize}" class="flex items-center gap-x-3 border border-gray-900/10 mt-1 rounded-md shadow-sm">                 
                        ...
                        <div class="relative w-full">
                            ...
                            <div class="absolute inset-y-0 left-0 flex items-center">                                                                
                                <select id="lada" name="lada" autocomplete="lada" class="h-full rounded-md border-0 bg-transparent py-0 pl-3 text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-600 text-xs">
                                    <option value="+52">MX</option>                       
                                    <option value="+1">US</option>
                                </select>
                            </div>                            
                            
                            <input placeholder="${placeholder}" type="tel" name="${id}" id="input${idCapitalize}" class="rounded-md w-full py-2 pl-14 text-gray-900 border-0 text-sm leading-6">
        ...
        this.addEventListener('input', () => {
            const span = document.getElementById(`span${idCapitalize}`);
            const inputValue = document.getElementById(`input${idCapitalize}`);
            const ladaTelefono = document.getElementById(`lada`);
            span.textContent = `${ladaTelefono.value} ${inputValue.value}`;
        });
    }
    /**
