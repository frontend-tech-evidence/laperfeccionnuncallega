...
import { IPdfCotizacion } from '../../../modules/cotizacion/domain/IPdfCotizacion.js'
import { getAllCostosPaquete } from '../../../modules/cotizador/application/CostosPaquete.js'
import { cambiarDeDivisa } from '../../../modules/cotizador/application/Divisas.js'
import {
    checkLimitesPaquete,
    checkLimitesPaquetes,
} from '../../../modules/cotizador/application/LimitesPaquetes.js'
import {
    costosEnterprise,
    costosGrow,
    costosInstitutional,
    costosManufacturing,
} from '../../../modules/cotizador/domain/CostosPaquetes.js'
import {
    costosTablaGrow,
    costosTablaInstitutional,
    costosTablaManufacturing,
    costosTablaEnterprise,
} from '../../../modules/cotizador/domain/CostosTabla.js'
import { Dolar } from '../../../modules/cotizador/domain/Divisas.js'
import {
    IAtributosDeCostosDinamicosPaquetes,
    ICostosPaquete,
} from '../../../modules/cotizador/domain/ICostosPaquete.js'

import { ICostosDetallesTabla } from '../../../modules/cotizador/domain/ICostosTabla.js'
import { ILimitesPaquete } from '../../../modules/cotizador/domain/ILimitesPaquete.js'
import {
    limitesEnterprise,
    limitesGrow,
    limitesInstitutional,
    limitesManufacturing,
} from '../../../modules/cotizador/domain/LimitesPaquetes.js'
...
function togglePaqueteView(
    idContenedorElement: string,
    idBtnCotizarElement: string,
    idBtnContratarElement: string,
    idContainerActivacion: string,
    idContainerMigracion: string,
    idContainerCapacitacion: string,
    idContainerPrecioImplementacion: string,
    idContainerPrecioBase: string,
    idContainerFacturacionAnual: string,
    idContainerPrecioParaIniciar: string,
    idErrorSpan: string,
    ...
    const btnContratar = document.getElementById(
        idBtnContratarElement
    ) as HTMLButtonElement

    const error = document.getElementById(idErrorSpan) as HTMLSpanElement

    const contenedorActivacion = document.getElementById(
        idContainerActivacion
    ) as HTMLImageElement

    const contenedorMigracion = document.getElementById(
        idContainerMigracion
    ) as HTMLImageElement

    const contenedorCapacitacion = document.getElementById(
        idContainerCapacitacion
    ) as HTMLImageElement

    const contenedorPrecioImplementacion = document.getElementById(
        idContainerPrecioImplementacion
    ) as HTMLImageElement

    const contenedorPrecioBase = document.getElementById(
        idContainerPrecioBase
    ) as HTMLImageElement

    const contenedorFacturacionAnual = document.getElementById(
        idContainerFacturacionAnual
    ) as HTMLImageElement

    const contenedorPrecioParaIniciar = document.getElementById(
        idContainerPrecioParaIniciar
    ) as HTMLImageElement

    btnCotizar.classList.toggle('hidden', !enable)
    btnContratar.classList.toggle('hidden', !enable)
    error.classList.replace(
        'text-white',
        !enable ? 'text-red-400' : 'text-white'
    )
...
function printLabelDePaqueteRecomendado(
    atributosDinamicos: IAtributosDeCostosDinamicosPaquetes,
    ...limitesPaquetes: ILimitesPaquete[]
) {
    let paqueteRecomendado = limitesPaquetes.find(
        (paquete) =>
            atributosDinamicos.cantidadUsuariosRequeridos <=
    ...
    if (paqueteRecomendado) {
        limitesPaquetes.forEach((paquete) => {
            const elemento = document.getElementById(
                `containerRecomendacion${paquete.nombrePaquete}`
            )
...
function reloadLabelDePaqueteRecomendado() {
    ...
    if (
        atributosDinamicos.cantidadEmpleadosAgregados > 1 ||
        ...
        printLabelDePaqueteRecomendado(
            getAllAtributosDinamicosDeCotizador(),
            limitesGrow,
            limitesInstitutional,
            limitesManufacturing,
            limitesEnterprise
...
function getAllAtributosDinamicosDeCotizador(): IAtributosDeCostosDinamicosPaquetes {
    const timbresAgregados = document.getElementById(
        'Timbres'
    ) as HTMLInputElement
    const empleadosAgregados = document.getElementById(
        'Empleados'
    ) as HTMLInputElement
    const usuariosAgregados = document.getElementById(
        'Usuarios'
    ) as HTMLInputElement
    const sucursalesAgregados = document.getElementById(
        'Sucursales'
    ) as HTMLInputElement
    const isPagoImplementacionMensual = document.getElementById(
        'ImplementacionMensual'
    ) as HTMLInputElement
    const isPagoMembresiaMensual = document.getElementById(
        'MembresiaMensual'
    ) as HTMLInputElement
...
function printCostosPaquete(
    costosPaquete: ICostosPaquete,
    atributosDeCostosDinamicosCotizador: IAtributosDeCostosDinamicosPaquetes,
    nombrePaqueteCapitalize: string
): void {
    const costosPaquetes = getAllCostosPaquete(
        atributosDeCostosDinamicosCotizador,
        costosPaquete
    )
...
function checkLimitsPaquete(
    limitesPaquete: ILimitesPaquete,
    nombrePaqueteCapitalize: string,
    costosPaquete: ICostosPaquete
) {
    const atributosDinamicos = getAllAtributosDinamicosDeCotizador()

    const mensajeDeError = checkLimitesPaquete(
        atributosDinamicos.cantidadUsuariosRequeridos,
        atributosDinamicos.cantidadEmpleadosAgregados,
        atributosDinamicos.cantidadSucursalesAgregadas,
        limitesPaquete
    ...
    togglePaqueteView(
        `container${nombrePaqueteCapitalize}`,
        `btnCotizar${nombrePaqueteCapitalize}`,
        `btnContratar${nombrePaqueteCapitalize}`,
        `containerActivacion${nombrePaqueteCapitalize}`,
        `containerMigracion${nombrePaqueteCapitalize}`,
        `containerCapacitacion${nombrePaqueteCapitalize}`,
        `containerPrecioImplementacion${nombrePaqueteCapitalize}`,
        `containerPrecioBase${nombrePaqueteCapitalize}`,
        `containerFacturacionAnual${nombrePaqueteCapitalize}`,
        `containerPrecioParaIniciar${nombrePaqueteCapitalize}`,
        `error${nombrePaqueteCapitalize}`,
        mensajeDeError,
        enablePrint
    )
    ...
    const atributosDeCostos = enablePrint
        ...
        : {
              cantidadUsuariosRequeridos:
                  limitesPaquete.numeroDeUsuariosPermitidos,
              cantidadEmpleadosAgregados:
                  atributosDinamicos.cantidadEmpleadosAgregados,
              cantidadSucursalesAgregadas:
                  atributosDinamicos.cantidadSucursalesAgregadas,
              cantidadTimbresRequeridos:
                  atributosDinamicos.cantidadTimbresRequeridos,
              isPagoImplementacionMensual:
                  atributosDinamicos.isPagoImplementacionMensual,
              isModalidadRentaMensual:
                  atributosDinamicos.isModalidadRentaMensual,
...
function changeLabelsPagosPaquetesCards() {
    ...
    const { isModalidadRentaMensual, isPagoImplementacionMensual } =
        getAllAtributosDinamicosDeCotizador()
...
function setInformationFromPackageSelectedToLocalStorage(
    ...
    const { costoPrimerAno, costoSegundoAno, costoSiguienteUsuario } =
        getAllCostosPaquete(atributosDinamicosActuales, costosPaquete)
    const moneda = localStorage.getItem('moneda')
    ...
    const data: IPdfCotizacion = {
        ...
        cantidadSucursales:
            atributosDinamicosActuales.cantidadSucursalesAgregadas,
        cantidadTimbres: atributosDinamicosActuales.cantidadTimbresRequeridos,
        cantidadEmpleados:
            atributosDinamicosActuales.cantidadEmpleadosAgregados,
        precioPrimerAno: costoPrimerAno,
        precioSegundoAno: costoSegundoAno,
        isModalidadRentaMensual:
            atributosDinamicosActuales.isModalidadRentaMensual,
        isPagoImplementacionMensual:
            atributosDinamicosActuales.isPagoImplementacionMensual,
